...
const Gallery = ({ isFullPage = false, onSelect, initialSelected = null }) => {
  ...
  const [authAction, setAuthAction] = useState(''); // 'save' | 'comment'
  const [comments, setComments] = useState(userState.comments);       // { [id]: [{text,time}] }
      ...
      <div style={{maxWidth:1240,margin:'0 auto'}}>
        ...
        <div className="preview-masonry">
          {featured.map((idol,idx)=>{
            ...
            <motion.div 
              ...
              onClick={() => navigate('/gallery', { state: { activeId: idol.id } })}
        ...
        <Reveal delay={.15}>
          <div style={{display:'flex',gap:14,flexWrap:'wrap',justifyContent:'center',marginTop:44,paddingTop:40,borderTop:`1px solid ${C.border}`}}>
            <CurvyButton primary onClick={() => navigate('/gallery')}>
              View Full Gallery
            </CurvyButton>
...
const ImmersiveFeed = () => {
  ...
  // FIX: Ensure initial image is correctly selected
  ...
  useEffect(() => {
    ...
    const gid = idol.gid; // stable gallery_item_id for Supabase
    
    (async () => {
      const count = await getIdolLikesCount(gid);
      setLikesCounts(p => ({...p, [gid]: count}));
    })();
    
    (async () => {
      const count = await getIdolSavesCount(gid);
      setSavesCounts(p => ({...p, [gid]: count}));
    })();
    
    (async () => {
      const data = await getIdolComments(gid);
      setRealComments(p => ({...p, [gid]: data}));
    })();
    
    if (isLoggedIn) {
      logViewEvent(gid, userId);
    }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#050402', overflow: 'hidden', color: '#fff', zIndex: 2000 }}>
      ...
      {!immersiveMode ? (
        ...
        >
          ...
          {filteredIdols.map((idol, idx) => (
            <div key={idol.id} onDoubleClick={() => handleDoubleTap(idol.gid)} style={{ height: '100vh', width: '100vw', scrollSnapAlign: 'start', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ position: 'absolute', bottom: 100, right: 16, display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center', zIndex: 3 }}>
                {[
                  { id: 'like', icon: <IconHeart filled={liked[idol.gid]}/>, count: likesCounts[idol.gid] ?? '0', action: () => toggleLike(idol.gid), active: liked[idol.gid] },
                  { id: 'comment', icon: <IconComment/>, count: (realComments[idol.gid]?.length ?? 0) || '0', action: () => setShowComments(true), active: false },
                  { id: 'save', icon: <IconBookmark filled={saved[idol.gid]}/>, count: savesCounts[idol.gid] ?? '0', action: () => toggleSave(idol.gid), active: saved[idol.gid] },
          ...
          >
            ...
            >
              {[
                ...
                { l: 'Purity',     v: activeItem.purity || 'Certified' },
                { l: 'Dimensions', v: activeItem.dims || 'On request' },
                { l: 'Stone',      v: activeItem.stone || 'None' },
                { l: 'Duration',   v: activeItem.duration || '45–60 Days' },
                { l: 'Origin',     v: activeItem.origin || 'Sowcarpet' },
              ].map((d, i) => (
                <motion.div 
                  key={d.l}
            ...
            >
              ...
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 13, color: 'rgba(255,255,255,0.85)', lineHeight: 1.7, margin: 0 }}>{activeItem.artisanNotes || 'Every detail from the Kireedam (crown) to Padmam (lotus base) is hand-chiselled with sacred mantras, ensuring spiritual authenticity alongside visual beauty.'}</p>
            </motion.div>
      ...
      <AnimatePresence>
        {showComments && activeItem && (() => {
          const gid = activeItem.gid;
          const allComments = (realComments[gid] || []).concat(comments[gid] || []);
          return (
              ...
              {allComments.length === 0 ? (
                ...
                allComments.map((c, i) => (
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i*0.05 }} key={c.id || i} style={{ marginBottom: 24, display: 'flex', gap: 16, background: 'rgba(255,255,255,0.02)', padding: 20, borderRadius: 16, border: '1px solid rgba(255,255,255,0.03)' }}>
                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,215,0,0.1)', border: '1px solid rgba(255,215,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFD700', fontFamily: "'Cinzel', serif", fontSize: 18, flexShrink: 0 }}>{((c.user || c.user_id || 'A') + '')[0].toUpperCase()}</div>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 8 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#FFF', fontFamily: "'Jost', sans-serif", letterSpacing: '0.05em' }}>{c.user || (c.user_id === userId ? 'You' : 'Devotee')}</div>
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', fontFamily: "'Jost', sans-serif" }}>{c.time || (c.created_at ? new Date(c.created_at).toLocaleDateString() : '')}</div>
                      </div>
                      <div style={{ fontSize: 16, fontFamily: "'Cormorant Garamond', serif", color: 'rgba(255,255,255,0.85)', lineHeight: 1.5, fontStyle: 'italic' }}>"{c.content || c.text}"</div>
                    </div>
                  </motion.div>
                ))
              )}
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh' }}>
        <Gallery isFullPage={true} initialSelected={initialId ?? null} onSelect={null} />
      </div>
      <Footer />
    </>
  );
        ...
        return (
           <div>
             <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
               <h3 style={{ fontFamily: "'Cinzel', serif", fontSize: 24, margin: 0, color: C.text }}>Artwork Management</h3>
               <button style={{ background: C.gold, color: '#000', border: 'none', padding: '10px 20px', borderRadius: 8, fontFamily: "'Jost', sans-serif", fontWeight: 700, cursor: 'pointer' }}>+ Upload Artwork</button>
             </div>
             <div style={{ background: C.surfaceWarm, border: `1px solid ${C.border}`, borderRadius: 16, padding: 40, textAlign: 'center', color: C.dim }}>
                Gallery grid management UI goes here.
             </div>
           </div>
        );
      default:
