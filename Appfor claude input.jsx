          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
              <button
                ...
                style={{
                  background: cardOpen
                    ? (C.isDark ? 'rgba(255,215,0,0.12)' : 'rgba(180,130,10,0.12)')
                    : 'none',
                  border: `1px solid ${cardOpen ? (C.isDark ? 'rgba(255,215,0,0.35)' : 'rgba(180,130,10,0.35)') : 'transparent'}`,
                  cursor: 'pointer',
              ...
              >
                ...
                <div style={{
                  ...
                  borderRadius: 2,
                  transform: cardOpen ? 'translateY(4.25px) rotate(45deg)' : 'none',
              ...
              <div style={{ width: 1, height: 26, background: sepColor, marginLeft: 8 }}/>
            </div>
  ...
  return (
    <section id="home" ref={ref} style={{position:'relative',height:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
      ...
      <motion.div style={{y:textY,opacity:fade,position:'relative',zIndex:3,textAlign:'center',padding:'0 24px',maxWidth:1080,width:'100%'}}>
        ...
        {/* Logo image replacing 3D model */}
        <motion.div initial={{opacity:0,scale:.75}} animate={{opacity:1,scale:1}} transition={{duration:1.6,delay:2.2,ease:[.16,1,.3,1]}}
          style={{width:220,height:220,margin:'0 auto 28px',position:'relative',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{position:'absolute',inset:'-30%',background:`radial-gradient(circle,rgba(255,200,50,.16) 0%,transparent 65%)`,pointerEvents:'none'}}/>
          <div style={{width:180,height:180,borderRadius:'50%',overflow:'hidden',position:'relative',
            border:`1px solid rgba(255,215,0,0.35)`,
            boxShadow:`0 0 40px rgba(255,215,0,0.18), 0 0 80px rgba(255,215,0,0.06), inset 0 0 30px rgba(0,0,0,0.5)`}}>
            <img src={LOGO_HERO} alt="Vijay Metal Works" loading="eager"
              style={{width:'100%',height:'100%',objectFit:'cover',filter:'brightness(1.05) contrast(1.02)'}}/>
          </div>
          {/* Orbiting decorative ring */}
          <motion.div animate={{rotate:360}} transition={{duration:25,repeat:Infinity,ease:'linear'}}
            style={{position:'absolute',inset:-14,borderRadius:'50%',
              border:`1px dashed rgba(255,215,0,0.2)`,pointerEvents:'none'}}/>
        </motion.div>
        ...
        <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:1,delay:3.05}}
          style={{display:'flex',gap:18,justifyContent:'center',flexWrap:'wrap',alignItems:'center'}}>
        ...
        <AnimatePresence>
          {sel && (
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:10}}
              style={{padding:'32px 36px',border:`1px solid ${C.borderHi}`,background:'rgba(255,255,255,0.03)',backdropFilter:'blur(10px)',marginBottom:24}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:16}}>
                <div>
                  <div style={{...ff.body,fontSize:7,letterSpacing:'.44em',color:'rgba(255,255,255,0.35)',fontWeight:600,textTransform:'uppercase',marginBottom:8}}>{sel.cat}</div>
                  <div style={{...ff.display,fontSize:28,color:C.text,fontWeight:700,marginBottom:6}}>{sel.deity}</div>
                  <div style={{...ff.serif,fontSize:16,color:C.dim,fontStyle:'italic'}}>{sel.metal} · Handcrafted</div>
                </div>
                <CurvyButton primary onClick={()=>window.open(`https://wa.me/919382877351?text=${encodeURIComponent(`Namaskaram, I am interested in commissioning a ${sel.deity} idol in ${sel.metal}. Please share details.`)}`)}>
                  Commission This →
                </CurvyButton>
              </div>
            </motion.div>
