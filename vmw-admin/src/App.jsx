    ...
    try {
      ...
      const insertRes = await fetch(`${SB_URL}/rest/v1/gallery_items`, {
        ...
        body: JSON.stringify({
          ...
          ...(form.artisan_notes?.trim() ? { artisan_notes: form.artisan_notes.trim() } : {}),
        }),
      });
      if (!insertRes.ok) {
      ...
      setForm({ title: '', category: 'Gold Work', metal_type: '24K Gold Nagas', is_featured: false, file: null, preview: null, artisan_notes: '' });
        ...
        {showForm && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div style={{ background: C.surface, border: `1px solid ${C.borderGold}`, borderRadius: 16, padding: 28 }}>
              ...
              {/* FIX: Artisan notes field — maps to artisan_notes column in gallery_items */}
              <div style={{ marginBottom: 20 }}>
                <label style={{ ...ff.body, fontSize: 11, color: C.dim, textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: 6 }}>Artisan Notes (optional)</label>
                <textarea value={form.artisan_notes || ''} onChange={e => setForm(f => ({ ...f, artisan_notes: e.target.value }))}
                  placeholder="Describe the craftsmanship, technique, or significance..."
                  rows={3}
