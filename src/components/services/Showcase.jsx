  ...
  <section id="showcase" className="section-pad" style={{position:'relative',zIndex:2,background:C.bg3,borderTop:`1px solid ${C.border}`}}>
    <div className="vmw-container">
      ...
      <div className="two-col" style={{display:'grid',alignItems:'center'}}>
        <SlideLeft>
          <span style={{...ff.body,fontSize:8,letterSpacing:'.52em',color:C.theme==='light'?'rgba(0,0,0,0.5)':C.dim,fontWeight:600,textTransform:'uppercase',display:'block',marginBottom:18}}>Masterpiece Gallery</span>
          <h2 style={{...ff.display,fontSize:'clamp(28px,5vw,62px)',lineHeight:.9,letterSpacing:'.04em',color:C.theme==='light'?'#1a1a1a':C.text,fontWeight:700,marginBottom:24}}>
            OUR SACRED<br/><span style={{color:C.theme==='light'?'#B8860B':'#FFD700'}}>CRAFT</span>
          </h2>
          <GoldRule w="56px" opacity={.5}/>
          <p style={{...ff.serif,fontSize:'clamp(15px,1.7vw,17px)',lineHeight:2.1,color:C.theme==='light'?'rgba(0,0,0,0.65)':C.dim,fontWeight:300,fontStyle:'italic',marginTop:28,marginBottom:20}}>
          ...
          <p style={{...ff.body,fontSize:'clamp(10px,1vw,11px)',lineHeight:1.95,color:C.theme==='light'?'rgba(0,0,0,0.5)':C.faint,fontWeight:300}}>
