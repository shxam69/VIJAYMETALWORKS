    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.bg1 }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ width: '100%', maxWidth: 400, background: C.surfaceWarm, border: `1px solid ${C.border}`, borderRadius: 24, padding: 40, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }}>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <input type="email" required placeholder="Admin Email" value={email} onChange={e=>setEmail(e.target.value)} style={{ padding: '14px 20px', background: C.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.9)', border: `1px solid ${C.border}`, borderRadius: 12, color: C.text, fontFamily: "'Jost', sans-serif" }}/>
          <input type="password" required placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} style={{ padding: '14px 20px', background: C.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.9)', border: `1px solid ${C.border}`, borderRadius: 12, color: C.text, fontFamily: "'Jost', sans-serif" }}/>
