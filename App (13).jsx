...
const SectionCTA = ({ primary="Commission a Piece", secondary="View Gallery", onPrimary, onSecondary }) => {
  ...
  <Reveal delay={.15}>
    <div style={{ display:'flex', gap:14, flexWrap:'wrap', justifyContent:'center', marginTop:52, paddingTop:44, borderTop:`1px solid ${C.border}` }}>
      ...
      <StarBorderButton onClick={onSecondary || (()=>{ window.scrollTo({top:0,behavior:'instant'}); navigate('/gallery'); })} speed={6}>
        {secondary}
      </StarBorderButton>
    </div>
  </Reveal>
...
const Gallery = ({ isFullPage = false, onSelect, initialSelected = null }) => {
  ...
  // FIX: Lock body scroll when lightbox is open — always restore on close/unmount
  useEffect(() => {
    if (selected !== null) {
...
/* ═══════════════════════════════════════════════════════════════
   IMMERSIVE FEED — Production-stable Instagram/TikTok gallery
   ROOT CAUSE FIXES:
   1. filteredIdols was recreated inline every render → useEffect
      with [filteredIdols] dependency fired on every render →
      3 async Supabase calls each time → setState in callbacks →
      re-render → repeat. Fixed with useMemo.
   2. Double-tap called handleInteraction → setShowAuthModal →
      global state update → re-render of ImmersiveFeed →
      scroll container lost scroll position → appeared frozen.
      Fixed: double-tap never opens auth modal.
   3. position:fixed container with overflow:hidden + no cleanup
      left body scroll locked after navigation. Fixed with useEffect
      cleanup.
   4. Multiple AnimatePresence blocks rendering overlapping fixed
      panels with pointer-events:auto caused gesture deadlocks.
      Fixed with explicit pointer-events management.
   5. Stale closure in handleDoubleTap: liked captured at mount.
      Fixed with useRef for liked state.
   6. Async IIFE calls inside useEffect had no cancellation →
      setState on unmounted component. Fixed with isMounted ref.
═══════════════════════════════════════════════════════════════ */
const ImmersiveFeed = () => {
  const C = useTheme();
  const navigate = useNavigate();
  ...
  // FIX 1: Memoize filteredIdols — prevents infinite rerender cascade
  const filteredIdols = React.useMemo(() => {
    const q = (searchQuery || '').toLowerCase();
    if (!q) return GALLERY_IDOLS;
    return GALLERY_IDOLS.filter(i =>
  ...
  // Compute initial index once from initialId (stable, not recalculated on every render)
  const initialIdxRef = useRef(() => {
    if (initialId == null) return 0;
    const idx = GALLERY_IDOLS.findIndex(i => i.id === initialId);
    return Math.max(0, idx);
  });
  const [activeIdx, setActiveIdx] = useState(() => initialIdxRef.current());
  const containerRef = useRef(null);
  const didScrollRef = useRef(false);

  // FIX 3: Cleanup body scroll on unmount — prevents scroll lock after navigate
  useEffect(() => {
    return () => {
      document.body.style.overflow = '';
  ...
  // Scroll to correct photo when entering immersive mode
  useEffect(() => {
    ...
    const scrollToIdx = () => {
      if (containerRef.current) {
        ...
        rafId = requestAnimationFrame(scrollToIdx);
      }
    };
    rafId = requestAnimationFrame(scrollToIdx);
  ...
  // Ref to track liked for stale closure fix
  ...
  // FIX 6: Async Supabase calls with isMounted guard — no setState on unmounted component
  useEffect(() => {
    ...
    getIdolLikesCount(gid).then(count => {
      if (isMounted) setLikesCounts(p => ({...p, [gid]: count}));
    });
    getIdolSavesCount(gid).then(count => {
      if (isMounted) setSavesCounts(p => ({...p, [gid]: count}));
    });
    getIdolComments(gid).then(data => {
      if (isMounted) setRealComments(p => ({...p, [gid]: data}));
    });
    if (isLoggedIn) logViewEvent(gid, userId);

    return () => { isMounted = false; };
  // Only re-run when the active item actually changes — NOT when filteredIdols array reference changes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeIdx, isLoggedIn, userId]);
  ...
  }, [activeIdx, filteredIdols.length]);
  ...
  const handleInteraction = useCallback((actionType) => {
    ...
    return true;
  }, [isLoggedIn, setAuthAction, setShowAuthModal]);

  const toggleLike = useCallback((gid) => {
    if (!handleInteraction('like')) return;
    const isCurrentlyLiked = likedRef.current[gid];
    setLiked(p => ({...p, [gid]: !isCurrentlyLiked}));
    setLikesCounts(p => ({...p, [gid]: (p[gid] || 0) + (isCurrentlyLiked ? -1 : 1)}));
    toggleIdolLike(gid, userId, isCurrentlyLiked);
    const state = getUserState();
    state.likes[gid] = !isCurrentlyLiked;
    setUserState(state);
  }, [handleInteraction, userId]);

  const toggleSave = useCallback((gid) => {
    if (!handleInteraction('save')) return;
    const isCurrentlySaved = saved[gid];
    setSaved(p => ({...p, [gid]: !isCurrentlySaved}));
    setSavesCounts(p => ({...p, [gid]: (p[gid] || 0) + (isCurrentlySaved ? -1 : 1)}));
    if (!isCurrentlySaved) {
      setSaveAnimMsg(true);
      if (saveAnimTimerRef.current) clearTimeout(saveAnimTimerRef.current);
      saveAnimTimerRef.current = setTimeout(() => setSaveAnimMsg(false), 2000);
    }
    toggleIdolSave(gid, userId, isCurrentlySaved);
    const state = getUserState();
    state.saves[gid] = !isCurrentlySaved;
    setUserState(state);
  }, [handleInteraction, saved, userId]);

  const postComment = useCallback((gid) => {
    if (!handleInteraction('comment')) return;
    if (!commentInput.trim()) return;
    const text = commentInput.trim();
    const username = user?.email?.split('@')[0] || 'Anonymous';
    setComments(p => ({...p, [gid]: [...(p[gid]||[]), { text, user: username, time: 'Just now', id: 'temp_' + Date.now() }]}));
    addIdolComment(gid, userId, text);
    setCommentInput('');
  }, [handleInteraction, commentInput, user, userId]);

  // FIX 2: Double-tap NEVER opens auth modal, NEVER causes freeze
  // Uses ref for liked to avoid stale closure
  const handleDoubleTap = useCallback((gid) => {
    if (doubleTapTimerRef.current) clearTimeout(doubleTapTimerRef.current);
    setDoubleTapGlow(true);
    doubleTapTimerRef.current = setTimeout(() => setDoubleTapGlow(false), 800);
    // Silently like if logged in — no modal, no state cascade
    if (isLoggedIn && !likedRef.current[gid]) {
      setLiked(p => ({...p, [gid]: true}));
      setLikesCounts(p => ({...p, [gid]: (p[gid] || 0) + 1}));
      toggleIdolLike(gid, userId, false);
      const state = getUserState();
      state.likes[gid] = true;
      setUserState(state);
    }
  }, [isLoggedIn, userId]);
  ...
  // FIX: Icons defined as stable components (moved outside render, but kept here as stable consts)
  // These do NOT cause re-renders as they are just SVG literals
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#050402', overflow: 'hidden', color: '#fff', zIndex: 2000 }}>
      ...
      {!immersiveMode ? (
        ...
        >
          ...
          {filteredIdols.map((idol, idx) => (
            <div key={idol.gid} onDoubleClick={() => handleDoubleTap(idol.gid)} style={{ height: '100vh', width: '100vw', scrollSnapAlign: 'start', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {/* Background Blurred Depth Image — no pointer events */}
              <img src={idol.img} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(50px) brightness(0.25)', transform: 'scale(1.2)', pointerEvents: 'none' }} alt="" />
              ...
              <motion.img 
                ...
                style={{ position: 'relative', maxWidth: '90vw', maxHeight: '80vh', objectFit: 'contain', zIndex: 1, filter: 'drop-shadow(0 20px 40px rgba(0,0,0,0.9))', borderRadius: 8, pointerEvents: 'none' }} 
              />
              
              {/* Double Tap Glow — pointerEvents:none prevents gesture deadlock */}
              <AnimatePresence>
              ...
              {/* LEFT SIDE: Title + metadata + View More */}
              <div style={{ position: 'absolute', bottom: 100, left: 24, maxWidth: 'calc(100% - 90px)', zIndex: 3 }}>
                ...
                >
                  ...
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, color: 'rgba(255,255,255,0.85)', margin: 0, textShadow: '0 2px 4px rgba(0,0,0,0.9)', fontStyle: 'italic', maxWidth: 400 }}>
                    {idol.artisanNotes ? idol.artisanNotes.slice(0, 80) + '…' : 'Sacred craftsmanship from our Sowcarpet workshop.'}
                  </p>
              ...
              {/* RIGHT SIDE: Actions — use gid consistently */}
              <div style={{ position: 'absolute', bottom: 100, right: 16, display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center', zIndex: 3 }}>
                {[
                  ...
                  <motion.div key={btn.key} initial={{ opacity: 0, x: 30 }} animate={{ opacity: idx === activeIdx ? 1 : 0, x: idx === activeIdx ? 0 : 30 }} transition={{ duration: 0.6, delay: 0.3 + i*0.1 }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                    <motion.button 
                      whileHover={{ scale: 1.15, filter: 'drop-shadow(0 0 8px rgba(255,215,0,0.5))' }} whileTap={{ scale: 0.9 }}
                      onClick={btn.action}
                      style={{ background: 'none', border: 'none', color: btn.active ? '#FFD700' : '#FFF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'color 0.3s', padding: 8 }}
                    >
                      {btn.icon}
                    </motion.button>
      ...
      {/* SHARE MENU — backdrop click dismisses; inner panel stops propagation */}
      <AnimatePresence>
        {showShareMenu && (
          <motion.div
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              style={{ background: 'rgba(20,18,16,0.97)', border: '1px solid rgba(255,215,0,0.3)', borderRadius: 24, padding: 40, display: 'flex', flexWrap: 'wrap', gap: 24, boxShadow: '0 24px 64px rgba(0,0,0,0.8)', maxWidth: 380 }}
            >
              <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <span style={{ fontFamily: "'Jost', sans-serif", fontSize: 12, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,215,0,0.8)', fontWeight: 700 }}>Share This Piece</span>
                <button onClick={() => setShowShareMenu(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 24, cursor: 'pointer', lineHeight: 1 }}>×</button>
              </div>
              {[
                { label: 'Copy Link', action: () => { navigator.clipboard.writeText(window.location.href); setShowShareMenu(false); } },
                { label: 'WhatsApp', action: () => window.open(`https://wa.me/?text=${encodeURIComponent(activeItem?.deity + ' — ' + window.location.href)}`) },
                { label: 'Instagram', action: () => {} },
                { label: 'Pinterest', action: () => {} },
              ].map(p => (
                <motion.button
                  key={p.label}
                  whileHover={{ y: -5, color: '#FFD700', borderColor: 'rgba(255,215,0,0.6)' }}
                  onClick={p.action}
                  style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', width: 72, height: 72, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, color: '#FFF', cursor: 'pointer', transition: 'all 0.3s', fontFamily: "'Jost', sans-serif", fontSize: 10, letterSpacing: '0.05em' }}
                >
                  <span style={{ fontSize: 20 }}>{p.label[0]}</span>
                  <span style={{ fontSize: 9, opacity: 0.7 }}>{p.label}</span>
                </motion.button>
              ))}
            </motion.div>
          </motion.div>
      ...
      {/* COMMENT DRAWER — FIX: uses activeItem.gid consistently; backdrop area closes drawer */}
      <AnimatePresence>
        {showComments && activeItem && (
          <>
            {/* Backdrop — tap outside drawer closes it without freezing */}
            <motion.div
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ position: 'fixed', inset: 0, zIndex: 148, background: 'rgba(0,0,0,0.4)' }}
              onClick={() => setShowComments(false)}
            />
            <motion.div
              initial={{ y: '100%' }} animate={{ y: '0%' }} exit={{ y: '100%' }}
              transition={{ type: 'spring', damping: 28, stiffness: 220 }}
              style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '65vh', background: 'rgba(15, 12, 10, 0.97)', backdropFilter: 'blur(40px)', borderTop: '1px solid rgba(255,215,0,0.15)', borderRadius: '32px 32px 0 0', zIndex: 149, display: 'flex', flexDirection: 'column', boxShadow: '0 -24px 80px rgba(0,0,0,0.8)' }}
              onClick={e => e.stopPropagation()}
            >
              <div style={{ padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <IconComment/>
                  <h3 style={{ margin: 0, fontSize: 14, fontFamily: "'Jost', sans-serif", letterSpacing: '0.15em', textTransform: 'uppercase', color: '#FFD700' }}>Admirations</h3>
                </div>
                <button onClick={() => setShowComments(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 28, cursor: 'pointer', lineHeight: 1 }}>×</button>
              </div>
              <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
                {((realComments[activeItem.gid] || []).concat(comments[activeItem.gid] || [])).length === 0 ? (
                  ...
                  ((realComments[activeItem.gid] || []).concat(comments[activeItem.gid] || [])).map((c, i) => (
                    <div key={c.id || i} style={{ marginBottom: 20, display: 'flex', gap: 16, background: 'rgba(255,255,255,0.02)', padding: 16, borderRadius: 14, border: '1px solid rgba(255,255,255,0.03)' }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(255,215,0,0.1)', border: '1px solid rgba(255,215,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFD700', fontFamily: "'Cinzel', serif", fontSize: 16, flexShrink: 0 }}>{(c.user || 'A')[0]}</div>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 6 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: '#FFF', fontFamily: "'Jost', sans-serif" }}>{c.user}</span>
                          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', fontFamily: "'Jost', sans-serif" }}>{c.time || (c.created_at ? new Date(c.created_at).toLocaleDateString() : '')}</span>
                        </div>
                        <div style={{ fontSize: 15, fontFamily: "'Cormorant Garamond', serif", color: 'rgba(255,255,255,0.85)', lineHeight: 1.5, fontStyle: 'italic' }}>{c.text || c.content}</div>
                      </div>
                    </div>
                  ))
                )}
              </div>
              <div style={{ padding: '16px 24px', background: 'rgba(0,0,0,0.6)', display: 'flex', gap: 12, paddingBottom: 'calc(16px + env(safe-area-inset-bottom))', flexShrink: 0 }}>
                <input
                  value={commentInput}
                  onChange={e => setCommentInput(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && postComment(activeItem.gid)}
                  placeholder="Share your admiration..."
                  style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,215,0,0.2)', borderRadius: 28, padding: '14px 20px', color: '#fff', outline: 'none', fontFamily: "'Jost', sans-serif", fontSize: 14 }}
                />
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  onClick={() => postComment(activeItem.gid)}
                  style={{ background: '#FFD700', color: '#000', border: 'none', borderRadius: 28, padding: '0 24px', fontWeight: 700, cursor: 'pointer', fontFamily: "'Jost', sans-serif", letterSpacing: '0.08em', textTransform: 'uppercase', fontSize: 12, flexShrink: 0 }}
                >Post</motion.button>
              </div>
            </motion.div>
          </>
...
const GalleryPage = ({ scrolled }) => {
  ...
  // Scroll to top when rendering the masonry view (not immersive)
  useEffect(() => {
    if (initialId == null) {
  ...
  // ImmersiveFeed handles both masonry and immersive viewer internally
  // It reads location.state.activeId to decide which mode to start in
...
const CommissionModal = ({ onClose, C }) => {
  ...
  // FIX: Always restore body scroll on unmount — prevents persistent scroll lock
  useEffect(() => {
...
const AuthModal = ({ onClose, action }) => {
  ...
  // FIX: Lock body scroll while modal is open, ALWAYS restore on unmount
  useEffect(() => {
    const prev = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    return () => {
