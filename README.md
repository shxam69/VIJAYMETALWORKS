...
### 🌐 Site Images — Admin-Controlled Homepage Images
Every image on the website can now be replaced from the admin panel without touching code.

**Setup (one-time):**
1. Go to Supabase SQL Editor → paste and run `SITE_IMAGES_PATCH.sql`
2. Open VMW Admin → **Site Images** tab

**How it works:**
- 30 image slots covering every section: Hero/CraftworkPanel, Services cards, Real Work Photos, Process steps, Gallery Preview, Showcase, and Legacy
- Click any card → select a new photo → click ⬆ Save → live on website instantly
- ↩ Reset button reverts any slot back to the original static image
- The website fetches overrides from the `site_images` Supabase table on load; falls back to static files if nothing is set
- Organised by section filter (Hero, Services, RealWork, Process, GalleryPreview, etc.)

### 🔐 Google Sign-In (Supabase OAuth)
Google sign-in is now fully wired via Supabase's OAuth redirect flow.

**Setup (one-time in Supabase):**
1. Go to Supabase Dashboard → **Authentication → Providers → Google**
2. Enable Google provider
3. Create OAuth credentials at [console.cloud.google.com](https://console.cloud.google.com) → APIs & Services → Credentials → Create OAuth 2.0 Client
...
6. Add your website domain to Supabase → Authentication → URL Configuration → Redirect URLs

**How it works:**
- Clicking "Continue with Google" in the Auth modal redirects to Google's consent screen via Supabase
- After consent, Google redirects back to `/gallery` with a token in the URL hash
- The app picks up the token, fetches the user profile, stores the session in `localStorage`, and signs the user in automatically
- Works alongside email/password — users can use either
...
| Admin tabs (Users, Collections, Comments, Analytics) | Show "module under development" — only Dashboard, Gallery, Site Images, Inquiries, Settings, Visitors are fully wired |
